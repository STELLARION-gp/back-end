...
export class SocketServer {
    ...
    this.io.on('connection', (socket: any) => {
      ...
      socket.on('join-group', async (groupId: number) => {
        try {
          ...
          const isMember = group.members.some(member => member.user_id === socket.userId);
          if (!isMember) {
      ...
      socket.on('send-message', async (data: {
        ...
        try {
          ...
          if (!groupId || !message?.trim()) {
            socket.emit('message-error', { error: 'Group ID and message are required' });
            return;
          }

          if (message.length > 2000) {
            socket.emit('message-error', { error: 'Message too long. Maximum 2000 characters.' });
            return;
          }
          ...
          if (!membership || !membership.group.is_active) {
            socket.emit('message-error', { error: 'Access denied or group inactive' });
            return;
          }

          if (membership.is_muted) {
            socket.emit('message-error', { error: 'You are muted in this group' });
            return;
          }
          ...
          const messageData = {
            ...
            user: {
              ...
              name: newMessage.user!.display_name || 
                    `${newMessage.user!.first_name || ''} ${newMessage.user!.last_name || ''}`.trim() ||
            ...
            reply_to: newMessage.reply_msg ? {
              ...
              user: newMessage.reply_msg.user ? {
                ...
                name: newMessage.reply_msg.user.display_name || 
                      `${newMessage.reply_msg.user.first_name || ''} ${newMessage.reply_msg.user.last_name || ''}`.trim() ||
      ...
      socket.on('add-reaction', async (data: {
        ...
        try {
          ...
          if (!messageId || !reaction?.trim()) {
            socket.emit('reaction-error', { error: 'Message ID and reaction are required' });
            return;
          }
          ...
          const isMember = message.group.members.some(m => m.user_id === socket.userId);
          if (!isMember) {
          ...
          socket.emit('reaction-error', { error: 'Failed to process reaction' });
        }
      });
      ...
      socket.on('edit-message', async (data: {
        ...
        try {
          ...
          if (newMessage.length > 2000) {
            socket.emit('edit-error', { error: 'Message too long. Maximum 2000 characters.' });
            return;
          }
          ...
          if (message.user_id !== socket.userId) {
            socket.emit('edit-error', { error: 'You can only edit your own messages' });
            return;
          }
          ...
          if (messageAge > maxEditTime) {
            socket.emit('edit-error', { error: 'Cannot edit messages older than 24 hours' });
            return;
          }
      ...
      socket.on('delete-message', async (data: { messageId: number }) => {
        try {
          ...
          const userMember = message.group.members.find(m => m.user_id === socket.userId);
  ...
  private async joinUserGroups(socket: any) {
    try {
      ...
      console.log(`🏠 ${socket.userName} auto-joined ${userGroups.length} groups`);
    } catch (error) {
